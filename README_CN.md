...
把你的微信聊天记录、B 站观看历史、YouTube 记录、小红书笔记喂给它——任何能反映你真实思维的数据。Percent 从中提取你的人格，生成结构化的 `core.md` 和可分享的 **Persona Card 人格卡片**，让任何大语言模型都能成为真正属于你的 AI，而不是千篇一律的通用助手。

- **隐私优先。** 原始数据保存在本地。使用云端模型（OpenAI/Claude/DeepSeek）时，文本片段会发送到对应提供商进行分析——使用 Ollama 可实现完全本地处理。
- **数据源无关。** 微信、YouTube、B 站、小红书——更多来源持续接入。
...
分析完成后，Percent 会生成一张 **Persona Card 人格卡片**：

- **8 维度星图** — 夜行性、回复惯性、表达锋利度、社交温差、情绪外显度、内容杂食度、品味独占欲、跨平台反差
- **人格标签** — AI 生成的人格原型名（如「深夜哲学家」）
- **个性洞察** — 从你的数据中提取的具体观察
- **PNG 导出** — 可分享的卡片图片

启动 Web 界面 `percent web`，进入卡片页面即可查看。
...
| YouTube | Google Takeout JSON/HTML | `percent import run youtube <路径>` |
| B 站 | 观看历史 JSON | `percent import run bilibili <路径>` |
| 小红书 | 浏览器导出（JS 脚本） | `percent import run xiaohongshu <路径>` |

**Beta：**

| 数据源 | 格式 | 命令 |
|--------|------|------|
| 微信数据库 | 解密后的 4.x SQLite | `percent import run wechat-db <路径>` |
| Telegram | JSON 导出 | `percent import run telegram <路径>` |
| WhatsApp | 聊天导出 txt | `percent import run whatsapp <路径>` |
| B 站 | Cookie 自动拉取 | `percent import bilibili --cookie` |
| YouTube | Cookie 自动拉取 | `percent import youtube --cookie` |
| Telegram | Telethon 自动拉取 | `percent import telegram --api-id` |

> Telegram 自动拉取需要安装：`pip install percent[telegram]`
...
percent import guide <来源>           查看数据源导出说明
percent import status                查看片段库统计
percent import bilibili --cookie     B 站 Cookie 自动拉取
percent import youtube --cookie      YouTube Cookie 自动拉取
percent import telegram --api-id     Telegram Telethon 自动拉取

percent persona view                 打印当前 core.md
percent persona stats                片段统计
percent persona rebuild              从所有片段重建 core.md
percent persona deep-analyze         跨源交叉验证 + 深度分析
percent persona big-five             计算大五人格分数
percent persona validate             运行 PersonaBench 评测

percent export soul                  生成 SOUL.md 系统提示
percent export core                  将 core.md 复制到指定路径

percent chat                         与你的人格交互对话
percent web                          启动 Web 界面

percent config llm                   修改 LLM 提供商 / 模型 / key
percent config cost                  查看各操作的 API 预估费用
